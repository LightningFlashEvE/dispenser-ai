...
- 管理库存、配方和操作日志
- 库存、配方、药品库、历史任务、日志和审计均通过 HTTP 分页/查询/修改获取；WebSocket 只用于变更通知或刷新提示，不传全量数据
...
- 称重曲线可以消费高频 `balance_reading` WebSocket 数据，但这些数据**只用于显示**。
- 前端必须分离三种节奏：WebSocket 数据接收、数字显示刷新、ECharts 渲染刷新；不要让每个原始采样点触发整页或整图重渲染。
- Dashboard 系统资源通过 HTTP 读取后端 cache；不要在前端用高频轮询补偿后端采样问题。
- 任务执行状态以 `command_id`、后端状态机、规则校验结果和控制层回调为准；WebSocket 只显示进度。
- 用户确认按钮只表达本机操作员确认，不能绕过后端规则校验。
- 急停和安全联锁由控制层/硬件优先保证；前端只显示状态和提供入口，不作为安全链路的唯一保障。
