...
> **模型**：Gemma 4 E4B / Qwen2.5-7B-Instruct / Phi-3 Mini（待选型测试后确定）
...
> 后端每次调用 Ollama 时，将 `{STATION_SNAPSHOT}` 替换为实时工位状态，其他字段固定。
...
补槽过程中，后端将历史轮次以 `messages` 数组传给 Ollama：

```python
messages = [
    {"role": "system", "content": SYSTEM_PROMPT},           # 含工位快照
    {"role": "user",   "content": "帮我配氯化钠"},            # 第1轮用户输入
    {"role": "assistant", "content": '{"intent_type":"dispense_powder","is_complete":false,...}'},
...
response = httpx.post(
    ...
    json={
        ...
        "stream": False,
        "format": "json",              # 强制 JSON 输出模式（Ollama 支持）
        "options": {
            "temperature": 0.1,        # 低温度，减少随机性，保证 JSON 格式稳定
            "num_predict": 512,        # 限制输出 token 数，intent_json 不需要很长
        },
...
| `format` | `"json"` | Ollama 的 JSON 模式，强制模型只输出合法 JSON |
| `num_predict` | 512 | intent_json 通常 200~400 tokens，留余量 |
| `keep_alive` | 300 | 半常驻，5 分钟内保持加载，0=立即释放 |
...
| 用户说配方名"PBS" | `intent_type: dispense_powder`，`reagent_hint.raw_text: "PBS"`，规则引擎查配方库 |
