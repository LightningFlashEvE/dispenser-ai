...
class AIExtractor:
    ...
    async def extract_patch(
        ...
        if self._llm is not None:
            patch = await self._extract_with_llm(task_type, current_draft, user_message)
        if patch is None:
            patch = _extract_with_rules(task_type, user_message)
        return _sanitize_patch(task_type, patch)
...
def _extract_with_rules(task_type: TaskType, text: str) -> dict[str, Any]:
    ...
    vessel_match = re.search(r"(?:放到|放入|到|容器|工位)\s*(?P<vessel>[A-Za-z]\d+|\d+号?工位)", text)
    if vessel_match:
...
def _extract_chemical_name(text: str) -> str | None:
    ...
    if match:
        ...
        name = re.sub(r"^\d+(?:\.\d+)?(?:mg|g|kg|毫克|克|千克)?", "", name, flags=re.I)
